    ...
    if (action === 'dashboard') {
      ...
      for (const c of ['COC', 'PXC', 'VXC']) {
        const [marketRows] = await pool.query(
          "SELECT price FROM ac_card_power_history WHERE currency = ? ORDER BY recorded_at DESC LIMIT 20",
          [c]
        );
      ...
      const [historyRows] = await pool.query(
        "SELECT currency, amount_vpln as vpln_amount, amount_crypto as crypto_amount, 'BUY' as type, invested_at as timestamp FROM ac_investments WHERE uuid = ? ORDER BY invested_at DESC LIMIT 10",
        [uuid]
      );
