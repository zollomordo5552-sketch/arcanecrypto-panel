    ...
    if (action === 'dashboard') {
      ...
      for (const c of ['COC', 'PXC', 'VXC']) {
        try {
          const [marketRows] = await pool.query(
            "SELECT price, recorded_at FROM ac_card_power_history WHERE currency = ? ORDER BY recorded_at DESC LIMIT 20",
            [c]
          );
          ...
          console.error("Market history error for", c, err);
        }
      }
      ...
      try {
        const [historyRows] = await pool.query(
          "SELECT currency, amount_vpln as vpln_amount, crypto_amount, 'BUY' as type, invested_at as timestamp FROM ac_investments WHERE uuid = ? ORDER BY invested_at DESC LIMIT 10",
          [uuid]
        );
