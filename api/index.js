  ...
  try {
    ...
    if (action === 'login') {
      ...
      const [rows] = await pool.query(
        "SELECT uuid FROM crypto_web_codes WHERE code = ? AND created_at > (NOW() - INTERVAL 15 MINUTE)",
        [code]
      );
      ...
      if (rows.length > 0) {
        ...
        await pool.query(
          "UPDATE crypto_web_codes SET code = ? WHERE uuid = ?",
          [token, uuid]
        );
    ...
    const [userRows] = await pool.query(
      "SELECT uuid FROM crypto_web_codes WHERE code = ?",
      [token]
    );
    ...
    if (action === 'dashboard') {
      ...
      // Gracz (VPLN)
      const [walletRows] = await pool.query("SELECT name, balance FROM crypto_wallets WHERE uuid = ?", [uuid]);
      if (walletRows.length > 0) {
        data.player.name = walletRows[0].name;
        data.economy.vpln = parseFloat(walletRows[0].balance);
      }

      // Saldo Crypto
      const [balanceRows] = await pool.query("SELECT currency, amount FROM crypto_balances WHERE uuid = ?", [uuid]);
      ...
      for (const c of ['COC', 'PXC', 'VXC']) {
        const [marketRows] = await pool.query(
          "SELECT price FROM crypto_market_history WHERE currency = ? ORDER BY timestamp DESC LIMIT 20",
          [c]
        );
      ...
      const [historyRows] = await pool.query(
        "SELECT currency, amount_vpln as vpln_amount, amount_crypto as crypto_amount, 'BUY' as type, timestamp FROM crypto_investments WHERE uuid = ? ORDER BY timestamp DESC LIMIT 10",
        [uuid]
      );
